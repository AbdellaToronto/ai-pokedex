...
export function PokemonComponent() {
  ...
  return (
    <div className="w-full p-6 space-y-6 bg-gradient-to-br from-blue-100 to-red-100 rounded-lg shadow-lg">
      ...
      <div className="relative overflow-hidden bg-white rounded-lg p-6 shadow-lg border-2 border-yellow-400">
        <div className="pokeball-pattern absolute inset-0 opacity-10"></div>

        <p className="relative text-gray-800 font-medium text-center break-words">
         Ask a question about a Pokémon! Be as in depth and technical as possible!
        </p>
        {outputData && <DynamicTable data={outputData} />}
      </div>
    </div>
  )
}